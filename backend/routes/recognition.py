...
from fastapi import APIRouter, Body, UploadFile, File, Form, Request, BackgroundTasks, HTTPException
from fastapi.responses import JSONResponse
from fastapi.concurrency import run_in_threadpool

import config
from services.database_service import get_all_face_encodings, get_user_by_id
...
@router.post("/api/recognize", tags=["Recognition"])
async def recognize_face(
    ...
    if file and file.filename:
        try:
            ...
            if contents:
                image_data = contents
                print(f"Processing uploaded file: {file.filename}, size: {len(contents)} bytes")
    ...
    # Try to get data from form data if neither file nor base64 was provided
        ...
        if "file" in form:
            ...
            if hasattr(file, "read"):
                ...
                if contents:
                    image_data = contents
                    print(f"Processing form file: {file.filename}, size: {len(contents)} bytes")
    ...
    if not image_data:
        raise HTTPException(
            status_code=400,
            detail="No image provided. Please upload a file or provide base64 image data."
        )
    ...
    async with recognition_semaphore:
        try:
            ...
            if image_array is None:
                raise HTTPException(
                    status_code=400,
                    detail="Invalid image data or format not supported"
                )
            ...
            db_face_encodings = await get_all_face_encodings()
                ...
                if db_face["face_encoding"]:
                    ...
                    if db_face.get("multi_angle_encodings"):
                        try:
                            ...
                            print(f"Error decoding multi-angle encodings: {e}")
            ...
            if best_match:
                # Get full user info
                user = await get_user_by_id(best_match["user_id"])
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error processing recognition request: {str(e)}"
            )

# For backward compatibility
@router.post("/recognize", tags=["Legacy"])
async def recognize_face_legacy(image_base64: str = Body(..., embed=True)):
    """Legacy endpoint for face recognition"""
    return await recognize_face(Request, None, image_base64)

@router.post("/api/analyze_face", tags=["Analysis"])
async def analyze_face(image_base64: str = Body(..., embed=True)):
    """
    Analyze a face from a provided image without registering it.

    Args:
        image_base64: Base64 encoded image

    Returns:
        Face analysis results including pose and features
    """
    # Validate input
    if not image_base64:
    ...
    try:
        ...
        if len(face_locations) > 1:
            return {
                "status": "success",
                "message": f"Multiple faces detected in the image ({len(face_locations)} faces). Analyzing the first face.",
                "face_detected": True,
                "multiple_faces": True,
                "face_count": len(face_locations)
            }

        # Analyze the face using thread pool
        face_analysis = await run_in_threadpool(
        ...
        if not face_analysis:
            return {
                "status": "success",
                "message": "Face detected but analysis failed. Please try a different image.",
                "face_detected": True,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error during face analysis: {str(e)}"
        )
