  ...
  ({ label, value, icon, monospace = false }) => {
    return (
      <div className="flex items-start">
        {icon && <div className="text-gray-500 mr-2 mt-1">{icon}</div>}
        <div>
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // We'll navigate in the useEffect
      } else {
        setDeleteError(true);
      ...
      if (!deleteError) {
        setTimeout(() => setDeleteConfirmOpen(false), 500);
      }
    }
  }, [userId, deleteUser, t, deleteError]);
